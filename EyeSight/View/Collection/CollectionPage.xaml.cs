...
namespace EyeSight.View.Collection
{
    /// <summary>
    ...
    public sealed partial class CollectionPage : Page
    {
        ...
        public CollectionPage()
        {
            ...
            if (collectionViewModel == null)
            {
                ...
                collectionViewModel = SimpleIoc.Default.GetInstance<CollectionViewModel>();
            }

            this.DataContext = collectionViewModel;
            ...
            this.szListView.ItemContainerStyle = AppEnvironment.IsPhone ? Application.Current.Resources["PhoneListViewItemStyle"] as Style : Application.Current.Resources["PCPastCategoryListViewItemStyle"] as Style;
            this.szFavoriteFlipView.Padding = AppEnvironment.IsPhone ? new Thickness(0, 0, 0, 0) : new Thickness(0, 0, 0, 3);
        ...
        private void szListPC_ViewChangeStarted(object sender, SemanticZoomViewChangedEventArgs e)
        {
            if (e.IsSourceZoomedInView)
            {
                Messenger.Default.Send<bool>(true, AppMessengerTokenConst.IS_FAVORITE_FLIPVIEW_SHOW);

                var model = DicStore.GetValueOrDefault<ModelPropertyBase>(AppCommonConst.CURRENT_SELECTED_ITEM
                    , null);
                if (model != null)
                {
                    this.szFavoriteFlipView.SelectedItem = model;
                }
            }
            else
            {
                Messenger.Default.Send<bool>(false, AppMessengerTokenConst.IS_FAVORITE_FLIPVIEW_SHOW);
            }
        }
        #endregion
